          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-3 max-w-3xl text-sm leading-7 text-neutral-400 md:text-base">
                当前登录账号为 {user.email || user.nickname}。这里直接查看平台运行状态、用户行为、调用频率、AI 使用量与反馈处理，不再嵌入用户前台界面。
              </p>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-white/8 bg-black/20 px-4 py-4">
              <div className="flex items-center gap-2 text-remuse-accent">
                <Sparkles size={16} />
                <span className="text-[11px] font-mono uppercase tracking-[0.24em]">Monitoring</span>
              </div>
              <p className="mt-3 text-sm text-neutral-300">总调用量、成功率、耗时趋势、功能分布与异常高频使用账号集中在这里查看。</p>
            </div>
            <div className="rounded-2xl border border-white/8 bg-black/20 px-4 py-4">
              <div className="flex items-center gap-2 text-cyan-300">
                <Shield size={16} />
                <span className="text-[11px] font-mono uppercase tracking-[0.24em]">User Detail</span>
              </div>
              <p className="mt-3 text-sm text-neutral-300">支持按邮箱、昵称、用户 ID 搜索，查看单用户最近行为与风险标记状态。</p>
            </div>
            <div className="rounded-2xl border border-white/8 bg-black/20 px-4 py-4">
              <div className="flex items-center gap-2 text-remuse-secondary">
                <Sparkles size={16} />
                <span className="text-[11px] font-mono uppercase tracking-[0.24em]">Feedback Queue</span>
              </div>
              <p className="mt-3 text-sm text-neutral-300">反馈列表与处理状态集中管理，不再和馆长办公室或内容模块混在一起。</p>
            </div>
          </div>
        </div>
      </div>
