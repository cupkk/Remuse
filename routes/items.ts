...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const { name, hallId, category, material, imageBase64, story, tags, ideas, status, isSample, dateCollected } =
      parsed.data;
    const id = uuidv4();
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const { name, hallId, category, material, imageBase64, story, tags, ideas, status, isSample } = parsed.data;
    ...
    const updated = updateItem({
      ...
      is_sample: isSample ?? existing.isSample,
    });
...
function resolveImageUrl<T extends { image_path?: string; imageUrl?: string }>(item: T | null | undefined) {
  if (!item) {
    return item;
  }
