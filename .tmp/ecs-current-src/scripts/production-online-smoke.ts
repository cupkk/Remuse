...
async function runAiChecks(options: ScriptOptions, session: SessionContext) {
  ...
  const ideas = await fetchJson(`${options.baseUrl}/api/ai/remuse-ideas`, {
    method: 'POST',
    headers,
    body: JSON.stringify({
      itemDescription: '奶茶玻璃瓶',
      material: '玻璃',
    }),
  });
  ...
  return {
    ...
    summary: {
      ...
      ideasCount: Array.isArray(ideas.body?.ideas) ? ideas.body.ideas.length : 0,
